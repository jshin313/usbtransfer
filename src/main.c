...
static usb_error_t handleBulkOut(usb_endpoint_t endpoint,
    ...
    usb_ScheduleBulkTransfer(endpoint, gfx_vbuffer[0], BUF_SIZE, handleBulkOut, gfx_vbuffer[0]);
    return USB_SUCCESS;
}
...
//                break;
//            }
            usb_ScheduleBulkTransfer(usb_GetDeviceEndpoint((usb_device_t)event_data, 0x02), gfx_vbuffer[0], BUF_SIZE, handleBulkOut, gfx_vbuffer[0]);
            break;
        }
...
void main(void) {
    ...
    /* Print a string */
    os_PutStrFull("Initialized USB Serial Successfully!");
